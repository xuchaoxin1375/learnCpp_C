...
上面这段话可以压缩为几个字符？
*/
/*为了解决将编码压缩的问题,需要编写二进制数转会为十进制数,以便转换为ASCII,继而通过putchar()将ASCII码将数据压缩后的数据编码输出*/
/*相反的,为了实现解码,将字符串的各字符的ASCII转换为01编码串即可*/

 //Huffman树的存储结构
#define n 15					//叶子数目
#define m 2*n-1					//树中结点总数
#define ZIP_bits 7 //压缩编码地位数
...
/*huffman树*/
void main()/*如果树足够高的话,字符(单词)的编码最短长度可以超过1*/
{
	/*内部声明两个函数:*/
	/*不是调用!!!1*/
...
{	//根据Huffman树T 求Huffman编码表H
	...
	/*打印初步编码结果:*/
	for (i = 0; i < n; i++)/*节点的权和节点的名称的 各自的先后关系是一一对应的.*/
		printf("第%d个字符%s的编码为%s\n", i + 1, H[i].str, H[i].bits);

	/*根据输入的节点名称序列,依次地查找编码,并收集起来*/
	printf("输入的节点名称序列,依次地查找编码,并收集起来:\n");
	char temp[MAXLEN];
	...
	/*将收集起来地编码以七个字节映射到一个字符上(八个地话有点打印不出东西),收集字符并打印结果*/
	printf("将收集起来的编码以七个字节映射到一个字符上(八个地话有点打印不出东西),收集字符并打印结果:\n");
	//int i = 0;
